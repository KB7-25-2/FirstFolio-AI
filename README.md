...
질문, 시나리오, 정답 선택지와 해설에 포함된 금액, 금리, 비율과 기간은
LLM Grounding 전에 검색 상위 5개 청크의 수치와 한 번 더 대조합니다. 검색
근거에 없는 금융 수치가 있으면 `grounding_validation` 단계에서 즉시 차단하고
해당 문장을 `unsupported_claims`에 기록합니다. 시나리오형 문제는 제약 조건만으로
정답 하나를 결정할 수 있어야 하며, 자금 사용 시점이나 유동성 조건이 부족한데
특정 상품을 가장 유리하다고 단정하지 않도록 Grounding 규칙으로 검증합니다.
...
#### Grounding 품질 개선 실제 검증

2026-08-04 Issue #43 개선 후 같은 주제의 `TRUE_FALSE`, `SINGLE_CHOICE`,
`SCENARIO`를 각각 1개씩 실제 생성했다. 배치 ID는
`af0a07f4-63c7-424a-8279-b999c56ea024`이며 OX와 4지선다는 생성·근거검증과
수동 검수를 통과했다. 시나리오는 검색 근거에 없는 `200만 원`, `2년`,
`2.5%`를 사용해 `grounding_validation` 단계에서 차단됐다. 실패 항목은
재시도하지 않았고 성공 2개, 실패 1개, 중복 0개로 JSONL에 기록됐다.

이 검증은 근거 없는 금융 수치를 포함한 시나리오가 이전처럼 성공 결과로
저장되지 않는지 확인하기 위한 표본이다. 더 다양한 문서와 표현에서의 검증은
검색·생성 품질 평가 데이터셋으로 계속 확장한다.
...
- 시나리오·정답·해설의 근거 없는 금융 수치 선차단
- 시나리오 조건의 정답 결정 충분성과 복수 정답 가능성 검증
