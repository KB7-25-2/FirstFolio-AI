...
현재 로컬 품질 평가 입력은 `data/local/text/`의 교과서 TXT 6개와
`data/local/news/`의 뉴스 TXT 10개다. `data/local/`은 Git에서 제외되는
개발 입력이며 운영 원문 저장소가 아니다.

다음 개발 항목:

```text
교과서 6개·뉴스 10개 재등록과 청크 확인
→ BM25·FAISS 재색인
→ 문서 유형별 평가 질문과 정답 청크 확장
→ Recall@5·MRR 재측정
→ 실패 검색어와 청킹 문제 분석
→ Recall@5 80% 이상 확인 및 RAG 1차 품질 기준선 확정
```
